        ...
        try {
            if (bundle != null) {
                ...
                for (aPdusObj in pdusObj) {
                    ...
                    val showSmsIntent = Intent(context, Practice4SmsReceiverActivity::class.java)
                    showSmsIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                    showSmsIntent.putExtra(Practice4SmsReceiverActivity.EXTRA_SMS_NO, senderNum)
                    showSmsIntent.putExtra(Practice4SmsReceiverActivity.EXTRA_SMS_MESSAGE, message)
